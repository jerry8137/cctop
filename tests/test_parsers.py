...
def test_empty_usage():
    ...
    log_entry = {"timestamp": "2026-01-04T10:00:05Z", "message": {}}

    usage = parser.extract_usage_data(log_entry)
    assert usage is None


def test_killed_agent_status_is_stopped_sooner():
    """Test that an agent inactive for 15 mins (not waiting) is STOPPED, not IDLE."""
    from src.cctop.parsers.aggregator import DataAggregator
    from src.cctop.models.agent import AgentStatus
    from datetime import datetime, timedelta

    aggregator = DataAggregator()

    # Mock data
    entries = [{"type": "user", "message": "hello"}]  # No end_turn, so not waiting

    # 15 minutes ago
    last_activity = datetime.now() - timedelta(minutes=15)

    status = aggregator._determine_agent_status(entries, last_activity)

    assert status == AgentStatus.STOPPED, (
        f"Status {status} should be STOPPED after 15 mins inactivity"
    )
